...
public class AddPersonFrame extends JFrame{
	...
	private static String[] groupArr= {"classmate", "family", "friend" ,"undefined"};
	...
	public AddPersonFrame(String title, ContactsFrame contactsFrame) {
		...
		groupPanel.add(Box.createHorizontalStrut(40));
		...
		submitBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent actionEvent) {
				...
				System.out.printf("name: %s, type: %s, phone: %s, gruop\n", name, type, phone, group);
				if(name.length()==0 || phone.length()==0) {
					...
					if(checkPhone(type, phone)) {
						...
						if(isAdd==false) {
							...
							int row = studentQueries.updateStudent(studentNow.getMemberID(), name, type, phone, group);
							if(row==1) {
							...
							int row = studentQueries.addStudent(name, type, phone, group);
							if(row==1) {
	...
	public void setGroupSelected() {
		if(isAdd) {
			groupComboBox.setSelectedIndex(3);	//預選為undefined
		}
	}
